...
Batch 0 建立所有后续 Cycle 共用的协议。Batch 1 已迁移 Foundation 正文并建立 Cycle 1–2 的计划态 Research Note；实验、Evidence、结论与 Support Assessment 均尚未产生。Cycle 3–18 正文仍未生成。
...
- [Reference Project Atlas](docs/07-Reference-Project-Atlas.md)（仅 Batch 1 active overlay 已刷新；legacy rows 待对应 Batch 迁移）

其中的 `Week` 与 `EXP-Wxx-yy` 在对应内容 Batch 迁移前保持历史形态，不代表 Cycle 正文已经生成。V4.1 Week 1–2 已迁移为 Cycle 1–2；旧实验 ID 只在 migration record 中保留为 historical plan ID。
...
- Batch 1 只创建 `cycle-01` 与 `cycle-02` 的计划态 Research Note；不创建空的实验或 Evidence 记录。
