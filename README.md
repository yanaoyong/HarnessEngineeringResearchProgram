...
Batch 0 建立所有后续 Cycle 共用的协议。Batch 1–4 已生成 Foundation、Claude Code Host、Codex Host 与 ZCode Host 正文，并建立 Cycle 1–7 的计划态 Research Note；实验、Evidence Claim、Enterprise Fact、结论与 Support Assessment 均尚未产生。Cycle 8–18 正文仍未生成。
...
- [Reference Project Atlas](docs/07-Reference-Project-Atlas.md)（Batch 1–4 active overlay 已刷新；legacy rows 待对应 Batch 迁移）

其他 legacy 文件中的 `Week` / `EXP-Wxx-yy` 在对应内容 Batch 迁移前保持历史形态，不代表后续 Cycle 正文已经生成。V4.1 Week 1–6 已迁移为 Cycle 1–6；Cycle 7 是 V4.2 新增内容，没有旧 Week / 实验 ID；旧实验 ID 只在 migration record 中保留为 historical plan ID。
...
- Batch 1–4 只创建 `cycle-01` 至 `cycle-07` 的计划态 Research Note 和 Source Registry entries；不创建 Experiment Run、`EVD-*` 或 `ENT-*`。
