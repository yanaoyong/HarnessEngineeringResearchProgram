...
- Run Metadata 的 `repository.commit` 绑定 task fixture；H0 / H1 通过相关 Rule / Skill / Check / Adapter revision、Configuration snapshot 与 implementation artifact ID 绑定。保存 prompt、task、Host、Provider、Model、trace、diff、test、Human intervention 与 rollback evidence。
...
Cycle 18 可复用 `SRC-HARNESS-012` 的 SWE-agent ACI paper 作为 interface / experiment theory 背景，但该论文不是 myharness Project Evidence、Host Contract 或本项目 Result，不参与候选胜负裁决。
