# PART IV · V4.2 Cross-host Harness Abstraction
...
Batch 6 只生成 Cycle 9 · Four-host Harness Abstraction，并将 V4.1 Week 7 迁移为计划态 V4.2 正文。四个主要 Host 保持为 Claude Code、Codex、ZCode 与 OpenCode；Provider 与 Model 不是第五个 Host。
...
- Claude Code、Codex、ZCode、OpenCode 在 Instruction / Rule、Skill、Tool、Permission、Hook / lifecycle、Agent / Subagent、Plugin / distribution、Session / State 与 Evidence / Completion 上的可观察语义映射；
...
- 使用社区逆向或非官方仓库形成 ZCode Runtime source architecture 结论；
...
| Cycle 3–8 已登记 Host 来源 | L2 / L3 scoped | 建立每个 Host 的 Contract / Source / Behavior readiness map | 计划态 `SRC-*` 不等于 `EVD-*`；Claude Code 不推测未公开 Runtime，Codex / OpenCode Source 必须固定 revision，ZCode Runtime Source Gate 保持 `NOT VERIFIED` |
...
5. 该 Host 所需的 Source Authority 边界：Claude Code 不要求不存在的 Runtime Source；Codex Source claim 固定 `openai/codex` commit；ZCode Gate 未通过时不创建 Runtime Source claim；OpenCode Source / Behavior agreement 还需建立执行 artifact 到 commit 的 provenance；
...
| Semantic Slice | Claude Code | Codex | ZCode | OpenCode | 需要的最小 Evidence |
|---|---|---|---|---|---|
...
- ZCode Runtime Source Authority Gate 未通过时没有 Source architecture claim；Codex / OpenCode Source claim 绑定固定 commit，OpenCode Source / Behavior agreement 满足 artifact provenance；
