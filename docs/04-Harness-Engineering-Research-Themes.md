...
<tr class="header">
<th><strong>核心研究问题<br />
什么样的 Skill 真正改变 Agent 行为，而不只是写得很好？</strong></th>
</tr>
</thead>
<tbody>
</tbody>
...
34. 从 code-review 或 expert-reviewer 中只选一个。

35. V1：当前版本。

36. V2：只强化 name / description / 适用场景，测试 Discovery / Trigger。

37. V3：Behavior Contract 实验，增加 Trigger、Preconditions、Required Context、Procedure、Evidence Required、Completion Criteria、Failure Route。

38. 3–5 个相近任务，尽量同模型、同 Host、相似复杂度。
...
<tr class="header">
<th><strong>核心研究问题<br />
• Change Artifact 怎么避免与真实代码状态漂移？<br />
• Change 怎么避免“撒谎”？</strong></th>
</tr>
</thead>
<tbody>
</tbody>
...
39. 选择一个曾经状态不一致或怀疑存在 artifact drift 的历史 Change。

40. 建立 Spec Requirement ↔ Task ↔ Git Diff / File / Symbol ↔ Test ↔ CI ↔ Summary Status 的人工表。

41. Round A：人工检查。

42. Round B：让 Claude / Codex 直接检查，不给额外结构。

43. Round C：给明确 Convergence Checklist；可选加入 CodeGraph。

44. 比较发现漂移数、漏报、误报、语义判断、确定性判断和 CodeGraph 增益。
...
<tr class="header">
<th><strong>核心研究问题<br />
README 修改、普通 Bug 和架构变更，真的应该走同一套十阶段吗？</strong></th>
</tr>
</thead>
<tbody>
</tbody>
...
- BMAD：README 的 Scale-Domain-Adaptive；docs/explanation/、docs/how-to/；src/bmm-skills/ 中定向搜索 quick flow、project complexity、planning depth、workflow status、workflow help。
...
45. 选择 README / 文档措辞修改、普通 Hook Bug、Plugin / Architecture Change 三类历史或安全任务。

46. 回顾完整 10 Stage：哪些 Artifact 真有价值，哪些步骤近乎形式，哪一个 Gate 真正发现问题？

47. 提出临时实验模型（例如 Light / Standard / Full），但不写入 myharness。

48. 模拟或在安全任务上实践，记录流程时间、Artifact 数量、Context、Human Checkpoints、漏掉风险、返工、False Completion。
...
<tr class="header">
<th><strong>核心研究问题<br />
一个长任务如何跨多个 Session 保持认知连续，同时避免携带所有历史噪声？</strong></th>
</tr>
</thead>
<tbody>
</tbody>
...
49. 设计一个不能单 Session 完成的真实独立子问题，例如 Codex Plugin 移植中的一项。

50. 分为 Session A Research、B Plan、C Implementation、D Review。

51. 比较 Mode 1 Resume、Mode 2 普通 prose summary、Mode 3 结构化 Handoff（Goal / Current State / Decisions / Evidence / Files & Symbols / Completed / Next Step / Open Risks / Rejected Approaches）、Mode 4 现有 Changes Artifact。

52. 可在一个连续任务中分段采用不同方式，不要求重复四个完整项目。
...
<tr class="header">
<th><strong>核心研究问题<br />
Agent 发现的一个事实，什么时候值得永久进入 Harness？</strong></th>
</tr>
</thead>
<tbody>
</tbody>
...
- Agent OS：README 核心能力；commands/agent-os/ 中定向寻找 standards discovery / deployment / index / spec；profiles/default/global/ 与 config.yml。
...
53. 从 myharness 抽取 10 个 Failure Record、Improvement、真实 Bug、Review 问题或 Agent 误行为。

54. 第一层：One-off / Recurring。

55. 第二层：Host Issue / Model Behavior / Project Knowledge / Process Problem / Deterministic Engineering Constraint。

56. 第三层决定：Rule / Skill / Hook / Wiki / Test / Change Template / Nothing。

57. 对 3 个准备新增能力的案例估算 Recurring Value、Context Cost、Maintenance Cost、Cross-host Portability、Behavior Complexity。
