...
- 可选 CodeGraph 只能作为独立 `EXPLORATORY` / `OBSERVATION_ONLY` stratum，使用 Observation Outcome，不能混入 B / C 主裁决或填写独立 Hypothesis Result。
...
- `EXP-C12-02` 的 Result Unit 是 `TASK_INSTANCE`，Stratum Key 是 `task_instance_id`；两个 T03 strata 在同一个 Experiment Record 中分别裁决，Experiment-level Result 标为不适用。
...
`EXP-C13-01` 的 Outcome Mode 是 `OBSERVATION_ONLY`：它以 `COMPLETE / PARTIAL / INVALIDATED` 记录 Observation Outcome，Experiment Result 为 `NOT APPLICABLE · OBSERVATION ONLY`，不把 phase / mode 混杂写成 `INCONCLUSIVE` Hypothesis Result。
...
- 真实执行后，任何进入 Cycle 15/16 的可复用 ratification claim 必须派生 scoped `EVD-*`，回链 packet、evaluator reference 与 Run。
