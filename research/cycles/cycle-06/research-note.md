...
- `EXP-C06-01`：`EXPLORATORY` · `T01 · Engineering Constraint` · Non-destructive Execution Boundary Trace。
...
- 共同 fixture：受挑战的本地 acceptance command 在 control preflight 中会先向隔离实验目录写入内容固定、初始 absent 且可清理的 marker，再运行本地检查；Host hard-deny 必须稳定阻止该 marker，否则结果为 `INCONCLUSIVE`。
