# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-008

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-008`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Agent Skills
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/features/skills/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Agent Skills
...
- 本来源可支持的判断（Claim This Source May Support）：Skill location、SKILL.md、discovery / invocation 与 sharing Contract
- 本来源不可支持的判断（Claim This Source Cannot Support）：Skill 已发现、激活、执行、改善 outcome 或完全符合外部 Skill standard
