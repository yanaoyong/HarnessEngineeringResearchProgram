# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-010

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-010`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Extensions
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/extension/introduction/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Extensions
...
- 本来源可支持的判断（Claim This Source May Support）：Extension packaging、installation、scope、hot reload、Skill / Subagent / MCP distribution Contract
- 本来源不可支持的判断（Claim This Source Cannot Support）：第三方扩展可信、兼容行为等价、已激活、任务有效或企业允许
