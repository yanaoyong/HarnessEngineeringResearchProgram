# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-003

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-003`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Model Providers
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Model Providers
...
- 本来源可支持的判断（Claim This Source May Support）：Provider、endpoint / protocol、authentication、routing、Model selection 与 configuration boundary
- 本来源不可支持的判断（Claim This Source Cannot Support）：Provider / Model portability、相同 Model identity、请求行为等价或 Host performance
