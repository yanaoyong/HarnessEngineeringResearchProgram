# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-001

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-001`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code overview
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code overview
...
- 本来源可支持的判断（Claim This Source May Support）：installation、terminal-first product surface 与公开 capability overview
- 本来源不可支持的判断（Claim This Source Cannot Support）：当前安装行为、内部实现、企业批准、产品排名或 S1–S4
