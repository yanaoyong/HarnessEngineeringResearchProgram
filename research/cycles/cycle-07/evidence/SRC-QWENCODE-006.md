# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-006

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-006`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Sandbox
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/features/sandbox/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Sandbox
...
- 本来源可支持的判断（Claim This Source May Support）：sandbox providers、configuration、workspace isolation 与公开 platform boundary
- 本来源不可支持的判断（Claim This Source Cannot Support）：当前环境已隔离、完整安全保证、所有平台 parity 或组织批准
