# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-005

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-005`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Approval Mode
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/features/approval-mode/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Approval Mode
...
- 本来源可支持的判断（Claim This Source May Support）：approval modes、tool decision surface、mode switching 与公开 safety boundary
- 本来源不可支持的判断（Claim This Source Cannot Support）：本地 mode 已启用、classifier decision、Sandbox implementation、安全保证或绕过结论
