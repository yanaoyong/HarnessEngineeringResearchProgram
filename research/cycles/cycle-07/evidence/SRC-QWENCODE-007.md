# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-007

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-007`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Hooks
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/features/hooks/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Hooks
...
- 本来源可支持的判断（Claim This Source May Support）：Hook event、executor type、decision vocabulary、failure 与 lifecycle Contract
- 本来源不可支持的判断（Claim This Source Cannot Support）：Hook 已配置 / 触发、内部 event order、Behavior outcome 或跨 Host 等价
