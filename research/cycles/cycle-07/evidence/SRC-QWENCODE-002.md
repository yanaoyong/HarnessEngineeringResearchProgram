# 来源登记条目（Source Registry Entry）· SRC-QWENCODE-002

## 身份（Identity）

- 来源 ID（Source ID）：`SRC-QWENCODE-002`
- 来源角色（Source Role）：`OFFICIAL CONTRACT`
- 可产生的证据类型（May Produce Evidence Class）：`CONTRACT`
- 标题（Title）：Qwen Code Configuration / Settings
...
- 规范网址或仓库（Canonical URL or Repository）：https://qwenlm.github.io/qwen-code-docs/en/users/configuration/settings/
...
- Model：未绑定
- Runtime / Component Scope：Qwen Code Configuration / Settings
...
- 本来源可支持的判断（Claim This Source May Support）：configuration layers、settings surface、tool / privacy / telemetry configuration Contract
- 本来源不可支持的判断（Claim This Source Cannot Support）：本地配置已生效、默认值跨版本稳定、组织 policy 或 Behavior outcome
