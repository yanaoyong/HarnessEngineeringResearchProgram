...
本目录当前没有 implementation artifact、Run 或 `EVD-*`。实际执行必须分别登记 H0 / H1 Harness revisions、one-time build / preflight / rollback artifacts，以及每个候选的 A0 / A1 task Run、diff、test、trace、acceptance 与 Human intervention。Run 的 `repository.commit` 绑定共同 task-fixture baseline；Harness effect 绑定 Host、surface、Provider、endpoint / protocol、Model、Configuration 和 H0 / H1 artifact IDs。
