...
`EXP-C09-01` 只产生 capability question map 与 scoped observation，不裁决 `H-C09-01`，也不能把一次 trace 写成 capability support。
...
- Target strata：重算后的每个 target Host 单独保存 Experiment stratum、A / B Run group、Result 与 scoped `EVD-*`。
...
真实执行开始后，使用 [实验记录模板](../../../templates/experiment-record.template.md) 创建独立 Experiment Record，并为每次执行保存独立 Run Metadata。内容生成阶段不得创建 Run、Adapter implementation、Result、`EVD-*` 或 Support Assessment。
