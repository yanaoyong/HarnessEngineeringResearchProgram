...
- 范围外（Out of Scope）：Host surface parity 假设、未固定或无 artifact provenance 的 Qwen Code Runtime Source、公开 Model benchmark、企业 / 法律结论、完整 myharness Adapter 实现、Cycle 10–18。
- 退出条件（Exit Criteria）：四宿主 readiness map 完整；每个非 Unknown matrix cell 绑定 scoped `EVD-*`；执行 `EXP-C09-01`；`EXP-C09-02` 至少完成两个目标 Host strata 的独立 A / B Result，并按预注册规则决定是否形成 cross-host Experiment Result；明确 Direct / Adapted / Degraded / Host-specific / Unsupported / Unknown；形成 Mental Model V1。
...
四宿主官方来源复用 [Cycle 3–4 Claude Code](../cycle-04/research-note.md)、[Cycle 5–6 Codex](../cycle-06/research-note.md)、[Cycle 7 Qwen Code](../cycle-07/research-note.md) 与 [Cycle 8 OpenCode](../cycle-08/research-note.md) 已登记的 `SRC-*`。复用 Source ID 不会把计划来源提升为 Evidence Claim。
...
尚未登记。`SRC-CROSSHOST-004`、`SRC-CROSSHOST-005` 的 commit 均为 `NOT PINNED`。Codex、Qwen Code、OpenCode Source claim 分别受 Cycle 5–6、Cycle 7、Cycle 8 的 revision / provenance gate 约束；Qwen Code repository identity 已验证，但 commit 与 execution artifact mapping 均未固定。
...
- `EXP-C09-01`：`EXPLORATORY` · `OBSERVATION_ONLY` · `T01 · Engineering Constraint` · Four-host Semantic Capability Trace。
...
- Host strata：Claude Code、Codex、Qwen Code、OpenCode 各自使用独立 Run Metadata；只研究一个绑定 surface。Provider / Model 不能相同时不比较 outcome quality。
...
- Source / target derivation：`source_host` 必须是四宿主之一且具备可复查 Behavior baseline；`target_hosts = four_hosts - {source_host}`。默认 Claude 为 source 时，目标为 Codex、Qwen Code、OpenCode；Route Review 更换 source 时必须同时重算 targets，不得重合、遗漏或引入第五个 Host。
- Target strata：`EXP-C09-02` 的 Result Unit 为 `HOST`、Stratum Key 为 `target_host`；重算后的每个 target Host 独立 A / B Run group、Result 与 scoped `EVD-*`。
...
- Outcome / Result：均尚未运行；`EXP-C09-01` 执行后记录 Observation Outcome，Experiment Result 为 `NOT APPLICABLE · OBSERVATION ONLY`；`EXP-C09-02` 才填写 stratum 与 cross-host Hypothesis Result。
...
- Claude Code 未公开 Runtime Source 不应被其他项目实现替代；Codex / Qwen Code / OpenCode 默认分支不产生可复现 Source Evidence；Qwen Code / OpenCode 缺少 execution-artifact provenance 时，Source / Behavior agreement 保持 Unknown。
