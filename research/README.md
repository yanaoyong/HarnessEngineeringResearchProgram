...
> Batch 0 已冻结公共协议；Batch 1 的 C01–C02 正文与计划态 Research Note 已生成，研究执行尚未开始。
...
Batch 1 已创建两个非空的计划态 Cycle workspace：
...
V4.1 正文中的 `Week`、`week-xx/` 示例和 `EXP-Wxx-yy` 是迁移来源。Week 1–2 已迁移；其旧实验 ID 只作为 historical plan mapping 保留。旧 Evidence ID 不重新编号；新制品不得继续使用 Week 命名。
